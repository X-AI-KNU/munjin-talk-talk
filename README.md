...
의료 도메인에서 LLM을 진단 주체로 사용하는 것은 위험합니다. 문진톡톡은 LLM의 역할을 환자 발화를 정리하고 구조화하는 보조 처리기로 제한하고, 실제 의료 판단은 의료진에게 남기는 구조로 설계했습니다.

- LLM이 생성한 임의 `score`, `confidence`, `probability` 값은 사용하지 않습니다. 의료진 화면에는 진단 확률처럼 오해될 숫자를 노출하지 않고, "매칭됨", "우선 확인"처럼 확인이 필요한 상태만 표시합니다.
- `source_quote`는 환자 원문에 실제로 존재해야 합니다. 원문에 없는 표현을 만들면 검증 단계에서 실패로 처리합니다.
- JSON 결과는 미리 정의한 Pydantic 스키마를 통과해야 합니다. 필수 필드, enum, 타입, 허용되지 않은 추가 필드를 모두 검증합니다.
- 증상 매칭은 LLM의 자유 판단으로 확정하지 않습니다. LLM이 만든 증상 후보는 원천 JSON 기반 Hybrid IR(BM25 + Titan Vector + label bridge)을 거쳐 표준 증상명과 연결됩니다.
- validator 실패를 정상 결과처럼 숨기지 않습니다. 재시도 후에도 실패하면 실패 상태를 남기고, 의료진 화면에서 재검토하거나 수동 확인할 수 있게 합니다.
- 위험 표현은 rule-based safety flag로 별도 감지하여 원페이퍼 상단에 우선 확인 항목으로 표시합니다.
