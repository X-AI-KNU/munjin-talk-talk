...
- 디지털정보격차 실태조사는 정보소외계층의 디지털 접근·역량·활용 수준을 일반국민 대비로 측정합니다. 병원 문진이 앱·웹 입력 중심으로 설계되면, 스마트폰을 보유하고 있어도 화면 조작과 문항 이해에 익숙하지 않은 고령 환자는 서비스 첫 단계에서 배제될 수 있습니다. ([e-나라지표 디지털정보격차 현황](https://www.index.go.kr/unity/potal/main/EachDtlPageDetail.do?idx_cd=1367))

기존 앱·웹 기반 문진은 작은 화면에서 여러 문항을 읽고 직접 입력해야 해 고령층 진입 장벽이 높고, 단순 LLM 챗봇은 *환각·임의 진단*이라는 의료 영역에서 치명적인 위험을 안고 있습니다.
